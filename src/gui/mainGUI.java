...
public class mainGUI extends JPanel {
    ...
    public mainGUI(Connection connection, CardLayout cardLayout, JPanel mainPanel, Users user) {
        ...
        // ==== Danh sách phim ====
        // Sử dụng GridBagLayout thay vì GridLayout để kiểm soát kích thước card
        ...
        try {
            centerContentPanel.add(new DetailFilm_GUI(connection, -1), "MovieDetail");
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void loadMovies(String keyword) {
        ...
        try {
            ...
            gbc.insets = new java.awt.Insets(10, 10, 10, 10); // Khoảng cách giữa các card
            gbc.anchor = GridBagConstraints.NORTHWEST; // Căn chỉnh card về phía trên trái
            int columnCount = 0;
            int rowCount = 0;
            ...
            for (Movies movie : movies) {
                ...
                movieCard.setPreferredSize(new Dimension(200, 350)); // Kích thước cố định
                ...
                // Thêm movieCard vào moviesPanel với GridBagLayout
                gbc.gridx = columnCount;
                gbc.gridy = rowCount;
                moviesPanel.add(movieCard, gbc);

                columnCount++;
                if (columnCount >= 4) { // Chuyển sang hàng mới sau 4 cột
                    columnCount = 0;
                    rowCount++;
                }
        ...
        // Cập nhật ScrollPane và các thành phần giao diện
